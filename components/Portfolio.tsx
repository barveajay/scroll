      ...
      <div className="max-w-[1400px] mx-auto px-8">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-24 gap-12">
          <div className="space-y-4">
            <span className="text-xs font-black tracking-[0.3em] uppercase text-brand-primary">The Archive</span>
            <h2 className="text-5xl md:text-8xl font-heading font-black tracking-tighter leading-none text-brand-white">
              SELECTED <br /><span className="text-brand-accent italic">WORKS.</span>
            </h2>
          </div>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-8 py-3 rounded-full text-[10px] font-bold tracking-widest uppercase transition-all ${
                  filter === cat 
                    ? 'bg-brand-primary text-brand-black shadow-[0_0_20px_rgba(139,197,63,0.3)]' 
                    : 'bg-white/5 text-gray-500 hover:bg-white/10 hover:text-brand-white'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="grid md:grid-cols-2 gap-px bg-white/5 border border-white/5 rounded-3xl overflow-hidden">
          {filteredProjects.map((project, idx) => (
            <div 
              key={project.id} 
              className={`group relative overflow-hidden bg-brand-black transition-all duration-1000 ${
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-brand-black via-transparent to-transparent opacity-80 group-hover:opacity-60 transition-opacity" />
              ...
              <div className="absolute inset-0 p-12 flex flex-col justify-end">
                <div className="translate-y-12 group-hover:translate-y-0 transition-transform duration-700 ease-out">
                  ...
                  <h3 className="text-4xl md:text-5xl font-heading font-bold mb-2 group-hover:italic text-brand-white transition-all">
                    {project.title}
                  </h3>
                  ...
                  <button className="w-14 h-14 rounded-full bg-brand-accent text-brand-black flex items-center justify-center scale-0 group-hover:scale-100 transition-transform duration-500">
                    <Plus size={24} />
                  </button>
                </div>
              </div>
