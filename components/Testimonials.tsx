      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-24 space-y-4">
          <div className="flex justify-center mb-4">
             <Network size={24} className="text-indigo-500 opacity-50" />
          </div>
          <h2 className="text-4xl md:text-5xl font-black font-heading tracking-tighter">TRUSTED BY VISIONARIES</h2>
          <p className="text-gray-500 uppercase text-[10px] tracking-[0.5em] font-bold">Network feedback verified</p>
        </div>

        <div className="grid md:grid-cols-3 gap-8">
          {TESTIMONIALS.map((t, idx) => (
            <div key={t.id} className="glass-card p-10 rounded-3xl relative group">
              <Quote className="text-indigo-600/10 absolute top-8 right-8 transition-colors group-hover:text-indigo-600/20" size={64} />
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-8 bg-indigo-500/30 group-hover:h-12 transition-all" />
              <p className="text-lg text-gray-300 font-light italic mb-8 leading-relaxed relative z-10">
                "{t.quote}"
              </p>
              <div className="flex items-center gap-4">
                <div className="relative">
                  <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full ring-1 ring-white/10 group-hover:ring-indigo-500/50 transition-all" />
                  <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-indigo-600 rounded-full border-2 border-neutral-950 flex items-center justify-center">
        ...
        <div className="mt-32 pt-20 border-t border-white/5 flex flex-wrap justify-center items-center gap-12 md:gap-24 opacity-20 grayscale transition-opacity hover:opacity-40 cursor-default">
          {['LOGOTYPE_01', 'LOGOTYPE_02', 'LOGOTYPE_03', 'LOGOTYPE_04', 'LOGOTYPE_05'].map((logo, i) => (
            <span key={i} className="text-xl font-heading font-bold tracking-tighter">{logo}</span>
          ))}
        </div>
      </div>
    </section>
